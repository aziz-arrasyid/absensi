                ...
                <div class="table-responsive rounded bg-white">
                    <table id="tableYearSemesters" class="table data-table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $loop->index + 1 }}</td>
                                <td>{{ $seksi->namaSeksi }}</td>
                                <td>
                                    <div class="btn-group btn-group-toggle btn-group-flat">
                                        <a class="button btn button-icon bg-warning editData" href="#" data-id="{{ $seksi->id }}" data-toggle="modal" data-target="#modal-edit-data">Edit</a>
                                        <a class="button btn button-icon bg-danger deleteData" data-id="{{ $seksi->id }}" href="#">Delete</a>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
                ...
                <div class="modal fade modal-add-data" tabindex="-1" role="dialog"  aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            ...
                            <form id="seksi-form" enctype="multipart/form-data" action="{{ route('data-seksi.store') }}" method="POST">
                                <div class="modal-body">
                                    @csrf
                                    <div class="form-group">
                                        <label for="namaSeksi">Nama Seksi :</label>
                                        <input type="text" name="namaSeksi" class="form-control" id="namaSeksi">
                                    </div>
                                </div>
...
<div class="modal fade modal-edit-data" tabindex="-1" id="modal-edit-data" role="dialog"  aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form id="seksi-form-edit" enctype="multipart/form-data">
                <div class="modal-body">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="namaSeksi-edit">Nama Seksi :</label>
                        <input type="text" name="namaSeksi" class="form-control" id="namaSeksi-edit">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
...
<script>
    $(document).ready(function() {
        ...
        $('.deleteData').on('click', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const data_seksi = $(this).data('id');
                    axios.delete(`/dashboard-admin/data-seksi/${data_seksi}`)
                    .then(() => {
                        Swal.fire(
                        'Terhapus!',
                        ...
                        'success'
                            ...
                            window.location.href = '{{ route('data-seksi.index') }}';
                        })
                    })
                    .catch(() => {
        ...
        $('.editData').on('click', function() {
            data_seksi = $(this).data('id');

            // fungsi pengambilan data dan ditampilkan start
            axios.get(`/dashboard-admin/data-seksi/${data_seksi}/edit`)
            .then(response => {
                $('#namaSeksi-edit').val(response.data.namaSeksi);
            })
            .catch(error => {
        ...
        document.getElementById('seksi-form-edit').addEventListener('submit', function(event) {
            event.preventDefault();

            const modal = $('#modal-edit-data');
            const formData = new FormData(this);
            axios.post(`/dashboard-admin/data-seksi/${data_seksi}`, formData)
            .then(response => {
                modal.modal('hide');
