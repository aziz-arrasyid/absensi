                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group btn-group-toggle btn-group-flat">
                                        <a class="button btn button-icon bg-warning editData" href="#" data-id="{{ $jabatan->id }}" data-toggle="modal" data-target="#modal-edit-data">Edit</a>
                                        <a class="button btn button-icon bg-danger deleteData" data-id="{{ $jabatan->id }}" href="#">Delete</a>
                                    </div>
                                </td>
                            </tr>
                            @endforeach
                ...
                <div class="modal fade modal-add-data" tabindex="-1" role="dialog"  aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            ...
                            <form id="jabatan-form" enctype="multipart/form-data" action="{{ route('data-jabatan.store') }}" method="POST">
                                <div class="modal-body">
                                    @csrf
                                    <div class="form-group">
                                        <label for="namaJabatan">Nama Jabatan :</label>
                                        <input type="text" name="namaJabatan" class="form-control" id="namaJabatan">
                                    </div>
                                </div>
...
<div class="modal fade modal-edit-data" tabindex="-1" id="modal-edit-data" role="dialog"  aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form id="jabatan-form-edit" enctype="multipart/form-data">
                <div class="modal-body">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="namaJabatan-edit">Nama Jabatan :</label>
                        <input type="text" name="namaJabatan" class="form-control" id="namaJabatan-edit">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
...
<script>
    $(document).ready(function() {
        ...
        $('.deleteData').on('click', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const data_jabatan = $(this).data('id');
                    axios.delete(`/dashboard-admin/data-jabatan/${data_jabatan}`)
                    .then(() => {
                        Swal.fire(
                        'Terhapus!',
                        ...
                        'success'
                            ...
                            window.location.href = '{{ route('data-jabatan.index') }}';
                        })
                    })
                    .catch(() => {
        ...
        $('.editData').on('click', function() {
            data_jabatan = $(this).data('id');

            // fungsi pengambilan data dan ditampilkan start
            axios.get(`/dashboard-admin/data-jabatan/${data_jabatan}/edit`)
            .then(response => {
                $('#namaJabatan-edit').val(response.data.namaJabatan);
            })
            .catch(error => {
        ...
        document.getElementById('jabatan-form-edit').addEventListener('submit', function(event) {
            event.preventDefault();

            const modal = $('#modal-edit-data');
            const formData = new FormData(this);
            axios.post(`/dashboard-admin/data-jabatan/${data_jabatan}`, formData)
            .then(response => {
                modal.modal('hide');
